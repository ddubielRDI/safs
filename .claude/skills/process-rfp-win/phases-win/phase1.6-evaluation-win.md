...
1. **EVALUATION_CRITERIA.json** exists at `{folder}/shared/EVALUATION_CRITERIA.json` — evidence: `ls -la` size > 500 bytes and parses as valid JSON

### Schema fidelity
2. **EVALUATION_CRITERIA.json top-level keys** include `extracted_at`, `selection_method`, `evaluation_factors`, `total_weight`, `recommendations`, `bid_structure_guidance`, `section_order_recommendations` — evidence: list actual top-level keys found
3. **Every evaluation_factor** has `factor_id`, `name`, `points`, `weight_normalized`, `detected_in_rfp` keys — evidence: print key set of factors[0]
4. **Weights sum** — `sum(f["weight_normalized"] for f in evaluation_factors)` is within ±2 of 100 — evidence: print actual sum
5. No `[:N]` slicing applied to deliverable content strings — evidence: grep for `\[:[0-9]+\]` in production code paths returned 0 hits

### Cross-stage consistency
6. **Selection method** is one of: "LPTA", "Best Value", "Quality-Based" — evidence: print `selection_method.type`
7. **Bid structure guidance** maps to the 3 standard bid sections (title-page.md, solution.md, timeline.md) — evidence: print keys of bid_structure_guidance dict
8. **Win themes** linked to at least 1 evaluation factor each — evidence: count factors with non-empty `bid_section_mapping.win_themes`
