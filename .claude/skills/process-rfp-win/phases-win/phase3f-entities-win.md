...
1. **ENTITY_DEFINITIONS.md** exists at `{folder}/outputs/ENTITY_DEFINITIONS.md` — evidence: `ls -la` size > 1,024 bytes

### Schema fidelity
2. **Entity Catalog section present** — grep "Entity Catalog" or "###" (entity name headings) returned >= 1 hit — evidence: grep result
3. **Data Dictionary Summary table present** — grep "Data Dictionary" or "| Entity | Attributes" returned >= 1 hit — evidence: grep result
4. **ERD diagram section present** — grep "Entity Relationship Diagram" or ERD-style notation returned >= 1 hit — evidence: grep result
5. No `[:N]` slicing applied to deliverable content strings — evidence: grep for `\[:[0-9]+\]` in production code paths returned 0 hits

### Cross-stage consistency
6. **40+ entities documented** — evidence: count `###` entity headings in ENTITY_DEFINITIONS.md; warn if < 40 (RFPs with few entities may score < 40 legitimately, report actual count)
7. **Attributes listed for each entity** — every entity section has an "Attributes" table — evidence: count entity sections without an attributes table (must be 0)
8. **Relationships inferred** for entities with foreign keys — evidence: count entity sections with "Relationships" subsection >= 1
